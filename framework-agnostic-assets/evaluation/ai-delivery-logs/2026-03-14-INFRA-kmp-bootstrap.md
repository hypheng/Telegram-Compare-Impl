...
| End | 2026-03-14 22:10:11 +0800 | 绝对时间 |
...
| 10 | 运行 `:shared-domain:allTests` / `:shared-data:allTests` / `:androidApp:assembleDebug` | partial | 依赖解析阶段暴露出残留 SOCKS 代理问题，因此新增 `./scripts/kmp-gradle-no-proxy.sh` 作为稳定入口 |
...
- Criteria still open:
  ...
  - 共享模块测试和 Android assemble 还没在无代理干扰的条件下完成一次完整基线
...
| tooling-gap | kmp | medium | 更重的依赖解析受残留 SOCKS 代理影响，Gradle 会尝试连接 `127.0.0.1:7890` | 新增 `./scripts/kmp-gradle-no-proxy.sh` 作为稳定入口 | - |
...
- Remaining:
  - 需要跑通首个真实切片
  - 需要拿到 `:shared-domain:allTests`、`:shared-data:allTests` 和 `:androidApp:assembleDebug` 的完整执行基线
