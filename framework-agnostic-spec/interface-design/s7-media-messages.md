...
- media picker 出现后应覆盖在消息区之上，不能把 top bar、消息区和 composer 整体向上挤压。
...
- 图片消息应首先读作“缩略图”，而不是“彩色标题卡”。
- 不在图片气泡内部放置醒目的 `已发送图片 / 收到图片` 文案徽标。
- caption 默认跟随图片底部弱化展示，不与图片标题抢主层级。
...
- composer 中的媒体入口应是图标式附件按钮，不使用厚重文本 chip 作为主要入口。
- 点击某个 fixture:
  - 立即发送该图片
  - sheet 关闭
  - 聊天详情显示发送成功反馈
