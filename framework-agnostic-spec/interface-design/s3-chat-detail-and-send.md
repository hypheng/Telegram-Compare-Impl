...
- 顶部导航优先使用 Telegram 风格的 `back + avatar + title/status` 信息层级，不在主导航里暴露切片编号或 debug 徽标。
- 消息区应使用 Telegram 风格的高密度气泡和轻背景，不使用卡片瀑布流。
- incoming 与 outgoing 气泡的色块、对齐和元信息层级要明显区分。
- 时间 / 发送状态应作为气泡内部的弱元信息贴近右下角，而不是单独拉成厚行。
- composer 使用圆角输入区和低噪音发送入口，媒体入口优先是图标式附件按钮，而不是显眼的文本 chip。
- 默认 fixture 至少要让消息区具备真实滚动长度，而不是只展示 2-3 条短消息。
- 短状态反馈优先使用居中的轻量状态胶囊，不使用厚重的整宽通知卡。
- debug-only 控件默认应折叠或弱化，不能和真实聊天主路径争夺视觉优先级。
